    ...
    <table className="min-w-full divide-y divide-gray-200 overflow-x-scroll">
      <thead className="bg-gray-50">
        <tr>
          ...
          >
            Password
            {sortConfig && sortConfig.key === 'password' && (
              <span className={`ml-1 ${sortConfig.direction === 'ascending' ? 'arrow-up' : 'arrow-down'}`} />
            )}
          </th>
      ...
      <tbody className="bg-white divide-y divide-gray-200">
        {rows.map((row) => (
          <tr key={row._id} className="hover:bg-gray-100">
            ...
            <td className="px-6 py-4 text-sm text-left text-gray-500 min-w-md">{row.password}</td>
            <td className="px-6 py-4 text-sm text-left text-gray-500 min-w-md">{row.role === 1 ? "Admin" : "User"}</td>
